...
function expandPhoneNullChecks(sql: string) {
  // Treat missing phone as NULL or empty/whitespace in unprocessed data.
  return sql.replace(
    /\b((?:[a-zA-Z_][\w]*)\.)?phone\s+is\s+null\b/gi,
    (_match, aliasPrefix: string | undefined) => {
...
function normalizeColumnNames(sql: string) {
...
function validateGeneratedSelect(rawSql: string): ValidationResult {
  const sql = expandPhoneNullChecks(
    normalizeColumnNames(normalizeTableName(normalizeSql(rawSql))),
  );
  if (!sql) {
