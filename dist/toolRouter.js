...
function expandPhoneNullChecks(sql) {
    // Treat missing phone as NULL or empty/whitespace in unprocessed data.
    return sql.replace(/\b((?:[a-zA-Z_][\w]*)\.)?phone\s+is\s+null\b/gi, (_match, aliasPrefix) => {
        const phoneRef = `${aliasPrefix ?? ""}phone`;
        return `(${phoneRef} IS NULL OR NULLIF(BTRIM(${phoneRef}), '') IS NULL)`;
    });
}
function normalizeColumnNames(sql) {
    // Common model drift to processed names; map to current unprocessed schema.
    return sql
        .replace(/\bprincipal\b/gi, "principle")
        .replace(/\binterest_rate\b/gi, "interest")
        .replace(/\bfull_name\b/gi, "name");
}
function normalizeTableName(sql) {
    // Common model mistakes for customer table naming.
    return sql
        .replace(/\b(from|join)\s+"?customer"?\b/gi, "$1 customers")
        .replace(/\b(from|join)\s+"?phone_numbers"?\b/gi, "$1 customers");
}
...
function validateGeneratedSelect(rawSql) {
    const sql = expandPhoneNullChecks(normalizeColumnNames(normalizeTableName(normalizeSql(rawSql))));
    if (!sql) {
    ...
    if (!/^(select|with)\b/i.test(single)) {
        return { ok: false, reason: "Only SELECT/CTE SELECT queries are allowed." };
    }
...
export async function handleToolCall2(generatedSql) {
